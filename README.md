...
## Inštalácia projektu
1. Najprv je potrebné vytvoriť si virtuálne prostredie pre inštaláciu potrebných knižníc. `python -m venv venv` (Pre niektoré operačné systémy je potrebné použiť namiesto `python` príkaz `python3`)
...
- `python manage.py makemigrations` - Ak vykonáte zmenu v definovaných modeloch (pridáte nové, upravíte existujúce), je potrebné aby Django vytvorilo migrácie, ktoré potom musíte zmigrovať do databázy.
